...
SELECT
  pg_size_pretty(pg_total_relation_size('vacuum_test'));
-- optional: Determine database size
SELECT
  pg_size_pretty(pg_database_size('analysis'));
...
-- Test its size again
SELECT
  pg_size_pretty(pg_table_size('vacuum_test'));
...
-- reload settings
  ...
  pg_dump - d analysis - U [user_name] - Fc > analysis_backup.sql -- Back up just a table
  pg_dump - t 'train_rides' - d analysis - U [user_name] - Fc > train_backup.sql -- Listing 17-12: Restoring the analysis database with pg_restore
