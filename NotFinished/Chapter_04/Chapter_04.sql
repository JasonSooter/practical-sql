...
COPY table_name
FROM
  'C:\YourDirectory\your_file.csv' WITH (FORMAT CSV, HEADER);
-- Listing 4-2: A CREATE TABLE statement for Census county data
  ...
  CREATE TABLE us_counties_2010 (
    ...
    -- County code
    area_land bigint,
    -- Area (Land) in square meters
    area_water bigint,
    -- Area (Water) in square meters
...
FROM
  'C:\YourDirectory\us_counties_2010.csv' WITH (FORMAT CSV, HEADER);
-- Checking the data
SELECT
  *
FROM
...
FROM
  'C:\YourDirectory\supervisor_salaries.csv' WITH (FORMAT CSV, HEADER);
-- Check the data
SELECT
  *
FROM
...
FROM
  'C:\YourDirectory\supervisor_salaries.csv' WITH (FORMAT CSV, HEADER);
INSERT INTO
  supervisor_salaries (town, county, supervisor, salary)
SELECT
  town,
  'Some County',
  supervisor,
...
-- Listing 4-7: Export an entire table with COPY
  COPY us_counties_2010 TO 'C:\YourDirectory\us_counties_export.txt' WITH (FORMAT CSV, HEADER, DELIMITER '|');
