...
FROM
  teachers
WHERE
  first_name LIKE 'S%' -- remember that LIKE is case-sensitive!
  AND salary > 40000;
...
--------------------------------------------------------------
  ...
  -- table. Why?
  -- Answer:
  numeric(4, 1) -- numeric(4,1) provides four digits total (the precision) and one digit after
  ...
  -- larger name column?
  -- Answer:
  varchar(50) -- 50 characters is a reasonable length for names, and varchar() ensures you
...
SELECT
  CAST('4//2017' AS timestamp with time zone);
--------------------------------------------------------------
  ...
  COPY actors
FROM
  'C:\YourDirectory\movies.txt' WITH (FORMAT CSV, HEADER, DELIMITER ':', QUOTE '#');
...
-- Note: You may never encounter a file that uses a colon as a delimiter and
  ...
  COPY (
    ...
    LIMIT
      20
  ) TO 'C:\YourDirectory\us_counties_housing_export.txt' WITH (FORMAT CSV, HEADER);
...
SELECT
  ...
  (CAST (p0010005 AS numeric(8, 1)) / p0010001) * 100 AS percent_american_indian_alaska_native_alone
FROM
  us_counties_2010
WHERE
...
FROM
  us_counties_2010 c2010
  RIGHT JOIN us_counties_2000 c2000 ON c2010.state_fips = c2000.state_fips
  AND c2010.county_fips = c2000.county_fips
WHERE
  c2010.geo_name IS NULL;
...
FROM
  pls_fy2014_pupld14a pls14
  JOIN pls_fy2009_pupld09a pls09 ON pls14.fscskey = pls09.fscskey
  JOIN obereg_codes ON pls14.obereg = obereg_codes.obereg
WHERE
  pls14.visits >= 0
  AND pls09.visits >= 0
GROUP BY
...
SELECT
  ...
  pls09.branlib
FROM
  pls_fy2014_pupld14a pls14 FULL
  OUTER JOIN pls_fy2009_pupld09a pls09 ON pls14.fscskey = pls09.fscskey
WHERE
...
FROM
  meat_poultry_egg_inspect;
-- view table with new empty columns
...
SELECT
  ...
  round(
    corr(median_hh_income, pct_masters_higher) :: numeric,
    2
  ) AS masters_income_r
FROM
  acs_2011_2015_stats;
...
SELECT
  ...
  popu_lsa,
  round(
    (visits :: numeric / popu_lsa) * 1000,
    1
  ) AS visits_per_1000,
  rank() OVER (
    ORDER BY
      (visits :: numeric / popu_lsa) * 1000 DESC
  )
FROM
  pls_fy2014_pupld14a
WHERE
...
SELECT
  round(
    corr(
      total_amount,
      (
        date_part('epoch', tpep_dropoff_datetime) - date_part('epoch', tpep_pickup_datetime)
      )
    ) :: numeric,
    2
  ) AS amount_time_corr,
  ...
  round(
    corr(total_amount, trip_distance) :: numeric,
    2
...
FROM
  nyc_yellow_taxi_trips_2016_06_01
WHERE
  tpep_dropoff_datetime - tpep_pickup_datetime <= '3 hours' :: interval;
...
-- 2. Using any one of the State of the Union addresses, count the number of
  ...
  WITH word_list (word) AS (
    SELECT
      regexp_split_to_table(speech_text, '\s') AS word
    FROM
      president_speeches
    WHERE
      speech_date = '1974-01-30'
  )
SELECT
  lower(
    replace(
...
WHERE
  search_speech_text @ @ search_query
ORDER BY
  rank_score DESC
LIMIT
  5;
...
SELECT
  statefp10 AS st,
  round (
    (sum(ST_Area(geom :: geography) / 2589988.110336)) :: numeric,
    2
  ) AS square_miles
FROM
...
-- 2. Using ST_Distance(), determine how many miles separate these two farmers’
  ...
  WITH market_start (geog_point) AS (
    SELECT
      geog_point
    FROM
      farmers_markets
    WHERE
      market_name = 'The Oakleaf Greenmarket'
  ),
  market_end (geog_point) AS (
    ...
    FROM
      farmers_markets
    WHERE
      market_name = 'Columbia Farmers Market'
  )
SELECT
  ST_Distance(market_start.geog_point, market_end.geog_point) / 1609.344 -- convert to meters to miles
FROM
  market_start,
  market_end;
...
SELECT
  ...
  markets.st
FROM
  farmers_markets markets
  JOIN us_counties_2010_shp census ON ST_Intersects(
    markets.geog_point,
    ST_SetSRID(census.geom, 4326) :: geography
  )
WHERE
  markets.county IS NULL
ORDER BY
  census.statefp10,
  census.name10;
...
-- 2. In Chapter 10, you learned how to calculate rates per thousand. Turn that
  ...
  CREATE
  OR REPLACE FUNCTION rate_per_thousand(
    observed_number numeric,
    base_number numeric,
    decimal_places integer DEFAULT 1
  ) RETURNS numeric(10, 2) AS $$ BEGIN RETURN round(
...
-- b) Create the function that the trigger will execute.
  CREATE
  OR REPLACE FUNCTION add_inspection_date() RETURNS trigger AS $$ BEGIN
UPDATE
  meat_poultry_egg_inspect
SET
  inspection_date = now() + '6 months' :: interval;
-- Here, we set the inspection date to six months in the future
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
AFTER
INSERT
  ON meat_poultry_egg_inspect FOR EACH ROW EXECUTE PROCEDURE add_inspection_date();
-- d) Test the insertion of a company and examine the result
INSERT INTO
  meat_poultry_egg_inspect(est_number, company)
VALUES
  ('test123', 'testcompany');
SELECT
  *
