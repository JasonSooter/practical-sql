...
SELECT
  ...
  round(
    (
      plants.plant_count / census.st_population :: numeric(10, 1)
    ) * 1000000,
    1
...
FROM
  ...
  JOIN (
    ...
    GROUP BY
      state_us_abbreviation
  ) AS census ON plants.st = census.state_us_abbreviation
ORDER BY
  plants_per_million DESC;
...
-- Listing 12-8: Using CTEs in a table join
  WITH counties (st, population) AS (
    SELECT
      state_us_abbreviation,
...
SELECT
  ...
  plants,
  round(
    (plants / population :: numeric(10, 1)) * 1000000,
    1
  ) AS per_million
FROM
  counties
...
FROM
  'C:\YourDirectory\ice_cream_survey.csv' WITH (FORMAT CSV, HEADER);
...
FROM
  'C:\YourDirectory\temperature_readings.csv' WITH (FORMAT CSV, HEADER);
